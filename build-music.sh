#!/usr/bin/env bash
# Build "The Music Lives On 8" (host / shared-library owner) into dist/.
# Run from the repository root. Uses the shared release.jks at the root.
set -euo pipefail
...
echo "==> [music] aapt2 compile"
"$AAPT2" compile --dir "$APP/res" -o "$BUILD/res.zip"

echo "==> [music] aapt2 link"
